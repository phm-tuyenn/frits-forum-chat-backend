...
router.post('/login', function(req, res) {
  let data = JSON.parse(Buffer.from(fs.readFileSync("./data/users").toString(), 'base64').toString('utf8'));
  data.forEach(user => {
...
router.post('/register', function(req, res) {
  ...
  if (!data.find((user) => {
    ...
    fs.writeFileSync("./data/users", Buffer.from(JSON.stringify(data), "utf8").toString('base64'));
